...
#define SRTB_CHECK_TEST_RING_BUFFER(expr) \
  SRTB_CHECK_TEST("[test-ring_buffer] ", expr)
...
int main() {
  ...
  sycl::queue q;
  srtb::memory::sycl_ring_buffer ring_buffer{
      srtb::host_allocator.allocate_unique<data_type>(buffer_length),
      buffer_length, q};
  ...
  SRTB_CHECK_TEST_RING_BUFFER(ring_buffer.push(mem, mem_length));
  SRTB_CHECK_TEST_RING_BUFFER(ring_buffer.pop(tmp, tmp_length));
  SRTB_CHECK_TEST_RING_BUFFER(
      check_absolute_error(tmp, tmp + tmp_length, mem, data_type{0}));

  SRTB_CHECK_TEST_RING_BUFFER(ring_buffer.push(tmp, tmp_length));
  SRTB_CHECK_TEST_RING_BUFFER(
